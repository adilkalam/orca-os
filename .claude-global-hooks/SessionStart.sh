...
# Check if .workshop directory exists in current project
if [ ! -d ".workshop" ]; then
    # Auto-initialize Workshop for this project
    echo ""
    echo "📝 Workshop: Initializing for this project..."
    workshop init --quiet 2>&1 | head -3

    # If init failed, exit silently
    if [ ! -d ".workshop" ]; then
        exit 0
    fi
fi
