...
**Goal:** Use SAME.dev’s cloning playbook (via web analysis) to produce a high‑fidelity UI/structure spec for a target site, then hand off to OS 2.2 pipelines (`/orca-nextjs` / `/orca-expo` lanes) for implementation. This command focuses on **spec and structure**, not blindly autogenerated code.
...
- For **Nextjs/web**:
  - Suggest running:
    - `/orca-nextjs "Implement clone of {url} ({clone_mode}) using spec at .claude/orchestration/evidence/clone-website-<slug>.md"`
    - This will trigger the Nextjs lane with its normal design-dna, standards, design QA, and verification gates.

- For **Expo/mobile**:
  - Suggest:
    - `/orca-expo "Implement Expo/mobile clone of {url} ({clone_mode}) using spec at .claude/orchestration/evidence/clone-website-<slug>.md"`
    - This will trigger the Expo lane, including design token, a11y, performance, and security gates.

Remind the user:
- This command creates a **structured spec and plan** from the target site.
- Actual implementation will go through the appropriate OS 2.2 lane pipelines with quality gates (design-dna/tokens, a11y, performance, security, verification), not as a one-off clone.
