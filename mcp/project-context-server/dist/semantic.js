 ...
 */
export class SemanticSearchImpl {
    projectIndexes = new Map();
    /**
     * Search for files semantically relevant to a query
     */
    async search(query, projectPath, maxResults = 10) {
    ...
    async indexProject(projectPath) {
        ...
        console.error(`Indexed ${index.files.size} files in ${projectPath}`);
    }
    /**
     * Recursively index a directory
     */
    async indexDirectory(dirPath, projectPath, index) {
     ...
     * Index a single file
     */
    async indexFile(filePath, lastModified, index) {
        try {
