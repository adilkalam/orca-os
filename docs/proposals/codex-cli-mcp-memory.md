# Proposal: Codex CLI — memory.search via MCP

Summary
- Add an MCP-backed retrieval path to Codex that prefers `memory.search` and falls back to ripgrep.
- Source of truth: per-project `.claude/memory/workshop.db` (shared with Claude Code).
...
Notes
- The MCP server resolves `.claude/memory/workshop.db` first, then legacy fallbacks. No manual path passing required.
