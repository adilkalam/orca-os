---
description: "Clone a website’s UI into an OS 2.0 project using SAME.dev-style web analysis and structured specs"
...
**Goal:** Use SAME.dev’s cloning playbook (via web analysis) to produce a high‑fidelity UI/structure spec for a target site, then hand off to OS 2.0 pipelines (`/orca` webdev/Expo lanes) for implementation. This command focuses on **spec and structure**, not blindly autogenerated code.
...
- Which **stack** they’re targeting (webdev vs Expo/mobile).
...
5. **Target Stack Plan**
   - If **webdev**:
     - Map spec to pages/routes/components in the webdev lane.
   - If **Expo/mobile**:
...
## 5. OS 2.0 Integration (Context & Memory)

After spec creation:

1. **ProjectContextServer:**
   - Call `mcp__project-context__query_context` with:
     - `domain`: `"webdev"` or `"expo"` (per target stack).
...
## 6. Handoff to Pipelines (/orca)

Finally, present the user with a clear next step:

- For **webdev**:
  - Suggest running `/orca "Implement clone of {url} ({clone_mode}) using spec at .claude/orchestration/evidence/clone-website-<slug>.md"` which will trigger the webdev pipeline.

- For **Expo/mobile**:
  - Suggest `/orca "Implement Expo/mobile clone of {url} ({clone_mode}) using spec at .claude/orchestration/evidence/clone-website-<slug>.md"` to trigger the Expo pipeline.

Remind the user:
- This command creates a **structured spec and plan** from the target site.
- Actual implementation will go through the appropriate OS 2.0 pipeline with quality gates (tokens, a11y, perf, etc.).
